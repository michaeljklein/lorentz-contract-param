...
t_letType :: Parser fp -> Parser (fp, Mi.Type)
t_letType fp = do
...
ct = (symbol "int" >> return Mi.CInt)
  ...
  <|> (symbol "key_hash" >> return Mi.CKeyHash)
...
t_map fp = (do symbol "map"; (f, t) <- fieldType fp; a <- comparable; b <- type_; return (f, Mi.Type (Mi.TMap a b) t))

t_big_map :: (Default a) => Parser a -> Parser (a, Mi.Type)
t_big_map fp = (do symbol "big_map"; (f, t) <- fieldType fp; a <- comparable; b <- type_; return (f, Mi.Type (Mi.TBigMap a b) t))
...
macro = do symbol' "CMP"; a <- cmpOp; Mi.CMP a <$> noteVDef
