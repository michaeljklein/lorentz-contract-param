...
type family FromT (t :: T) where
  FromT ('T_c a)      = FromCT a
  FromT 'T_unit       = ()
  FromT ('T_pair a b) = (FromT a, FromT b)
  FromT ('T_list a)   = [FromT a]
  FromT ('T_option a) = Maybe (FromT a)
  FromT ('T_or a b)   = Either (FromT a) (FromT b)
  FromT ('T_set k)    = Set (FromCT k)
  FromT ('T_map k v)  = Map (FromCT k) (FromT v)
  FromT 'T_key        = PublicKey
  FromT 'T_signature  = Signature
